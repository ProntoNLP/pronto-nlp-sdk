...
def SignIn(user, password):
    ...
    response = client.initiate_auth(AuthFlow='USER_PASSWORD_AUTH', AuthParameters=authParameters, ClientId='88nt7a64nkbukf7r49p8q82ht')
    ...
    try:
        ...
        while True:
            while iInFlight < iMaxParallel and iAt < len(sentences):
                ws.send(json.dumps({'authtoken': authToken, 'request': 'MacroLLM_ProcessSentence_Async', 'index': iAt, 'sentence': sentences[iAt]}))
                iAt += 1
                iInFlight += 1
