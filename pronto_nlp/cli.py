...
def main():
    ...
    if script_path:
        command_string = f'python {script_path} ' + " ".join([f"--{key} \"{value}\"" if key not in ('inputCSV', 'outputCSV') else f"\"{key}\"" for key, value in vars(args).items() if key not in ('command', 'sub_command') and value is not None])
